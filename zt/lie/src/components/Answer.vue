...
export default {
  ...
  created () {
    ...
    if (username && name === username) { // 如果是本人,跳转到结果页面
      ...
      const allAnswerIds = localStorage.getItem('allAnswerIds')
      if (allAnswerIds) {
        this.allAnswerIds = JSON.parse(allAnswerIds)
        const hasAnswered = this.allAnswerIds.some((answer) => {
  ...
  methods: {
    ...
    checkAnswer () {
      ...
      if (this.hasAnswered < this.question.answer.length) {
        this.nextStep = 'questionStart'
        this.judge()
      } else if (user.isMe(this.answerer) && user.isYou(this.answerer)) {
